...
- [ ] `category`와 `templateId`를 프로젝트 모델에 추가
- [ ] 업종 정의를 관리하는 category registry 작성
- [ ] 섹션 정의를 관리하는 section registry 작성
- [ ] 섹션 공통 인터페이스 정의: `schema`, `defaults`, `Editor`, `Renderer`
- [ ] 프로젝트별 섹션 순서와 활성 상태 저장
- [ ] 업종과 템플릿에 따른 기본 프로젝트 생성 함수 작성
- [ ] 기존 `ongyeol-cello` 데이터를 `lesson` 카테고리로 마이그레이션
...
| 2. 다중 카테고리 기반           | 🟡 진행 중 | CMS V2 범위, registry 구조, 체크리스트 작성   | category·section registry와 데이터 마이그레이션 |
...
- 다음 구현 대상: 프로젝트 모델, registry, 프로젝트 생성·전환
