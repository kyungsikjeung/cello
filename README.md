...
현재 CMS는 온결 첼로 스튜디오 한 곳에 최적화된 랜딩페이지를 편집합니다. 하지만 실제 제작 업무에서는 업종이 달라도 로그인, 프로젝트 관리, 이미지 업로드, 반응형 미리보기, 초안 저장, 공개, 버전 복원 같은 운영 기능이 반복됩니다.

CMS V2는 이 반복 영역을 공통 플랫폼으로 만들고, 회사 사이트·레스토랑·웨딩 페이지의 차이는 업종별 데이터 스키마와 섹션 템플릿으로 분리하는 것을 목표로 합니다. 하나의 거대한 편집 화면에 업종 조건문을 계속 추가하는 구조는 사용하지 않습니다.

### 2차 목표

한 CMS에서 프로젝트 생성 시 업종을 선택하고, 해당 업종에 필요한 메뉴·필드·미리보기·검증 규칙을 자동으로 구성해야 합니다.

- 회사 사이트: 회사 소개, 서비스, 포트폴리오, 팀, 문의 중심
- 레스토랑: 메뉴, 공간 사진, 영업시간, 예약, 위치 중심
- 웨딩 페이지: 신랑·신부 소개, 일정, 갤러리, 오시는 길, 참석 응답 중심
- 레슨/교육: 현재 첼로 페이지의 프로그램, 강사, 위치, 상담 구조 유지
...
- 업종 카테고리 선택과 프로젝트 생성
- 업종별 기본 콘텐츠와 메뉴 자동 생성
- 공통 섹션 레지스트리와 업종별 섹션 조합
- 회사·레스토랑·웨딩 기본 템플릿 각 1종
- 데스크톱·태블릿·모바일 실시간 미리보기
- 업종별 필수 필드와 공개 전 검증
- 기존 권한, 미디어, 초안, 공개본, 버전 기능 재사용
- 기존 첼로 프로젝트 데이터 마이그레이션

#### 이번 MVP에서 제외

- 드래그로 자유 배치하는 범용 페이지 빌더
- 고객이 HTML, CSS, JavaScript를 직접 삽입하는 기능
- 레스토랑 실시간 좌석·주문·결제 시스템
- 웨딩 축의금 결제와 계좌 중계
- 회사 채용 지원자 관리 시스템
- 다국어 번역 자동화
- AI 자동 사이트 생성
- Supabase/R2 운영 연결 완료 전 실제 고객 공개

### 구조 원칙

```mermaid
flowchart LR
    project["프로젝트"] --> category["업종 카테고리"]
    category --> registry["섹션 레지스트리"]
    registry --> schema["필드와 검증 스키마"]
    registry --> editor["CMS 편집 컴포넌트"]
    registry --> renderer["랜딩페이지 렌더러"]
    project --> content["프로젝트 콘텐츠"]
    content --> editor
    editor --> preview["반응형 미리보기"]
    renderer --> preview
    renderer --> publish["공개 사이트"]
```

- `category`: `lesson`, `company`, `restaurant`, `wedding` 중 하나를 가집니다.
- `template`: 같은 업종 안에서 레이아웃과 디자인 토큰 조합을 선택합니다.
- `section registry`: 섹션 이름, 데이터 스키마, 편집 UI, 렌더러, 기본값을 연결합니다.
- 프로젝트 콘텐츠는 섹션 순서와 섹션별 데이터를 저장합니다.
- 편집기와 공개 페이지는 같은 섹션 렌더러를 사용해야 합니다.
- 업종 전용 기능은 공통 스키마를 오염시키지 않고 해당 업종 모듈 안에 둡니다.

### 업종별 기본 섹션

| 업종      | 필수 섹션                                | 선택 섹션                           | 주요 CTA       |
| --------- | ---------------------------------------- | ----------------------------------- | -------------- |
| 레슨/교육 | Hero, Program, Teacher, Visit, Contact   | 후기, FAQ, 연주 갤러리              | 체험 레슨 문의 |
| 회사      | Hero, About, Services, Contact           | Cases, Team, Clients, Careers, News | 프로젝트 문의  |
| 레스토랑  | Hero, Menu, Hours, Location, Reservation | Chef, Gallery, Notice, Reviews      | 예약하기       |
| 웨딩      | Hero, Couple, Schedule, Location, RSVP   | Story, Gallery, Notice, Guestbook   | 참석 여부 전달 |

### 구현해야 할 MVP

#### A. 기반 구조

- [x] `category`와 `templateId`를 프로젝트 모델에 추가
- [x] 업종 정의를 관리하는 category registry 작성
- [x] 섹션 정의를 관리하는 section registry 작성
- [ ] 섹션 공통 인터페이스 정의: `schema`, `defaults`, `Editor`, `Renderer`
- [x] 프로젝트별 섹션 순서와 활성 상태 저장
- [ ] 업종과 템플릿에 따른 기본 프로젝트 생성 함수 작성
- [x] 기존 `ongyeol-cello` 데이터를 `lesson` 카테고리로 마이그레이션

#### B. 프로젝트 생성과 관리

- [ ] 새 프로젝트 만들기 화면
- [ ] 회사·레스토랑·웨딩·레슨 업종 선택
- [ ] 프로젝트명, 슬러그, 업종, 템플릿 입력 검증
- [ ] 접근 가능한 프로젝트 목록과 전환 기능
- [ ] 프로젝트 보관과 복원 기능
- [ ] 다른 프로젝트 콘텐츠와 미디어 격리 테스트

#### C. 공통 CMS 편집기

- [ ] registry를 읽어 왼쪽 메뉴 자동 구성
- [ ] 공통 텍스트·긴 글·숫자·색상·날짜·시간 필드
- [ ] 이미지·이미지 목록·링크·주소 필드
- [ ] 섹션 활성화·비활성화
- [ ] 허용된 범위에서 섹션 순서 변경
- [ ] 업종별 공개 전 필수 항목 점검표
- [ ] 초안과 공개본의 섹션 단위 변경 비교
...
- [x] IndexedDB 미디어 라이브러리
- [x] 원본 보관과 HEIC 표시용 변환
- [x] 프로젝트당 250MB·파일당 15MB·100개 제한
- [x] PC·모바일 초점 개별 설정
- [x] 사용 중인 미디어 삭제 방지
- [ ] 업종별 권장 이미지 비율 안내
- [ ] 갤러리 다중 선택과 순서 변경
- [ ] 프로젝트 복제 시 미디어 참조 복제 정책
- [ ] R2 원본·파생 이미지 저장 어댑터

#### H. 권한과 공개

- [x] 소유자·편집자·검토자 로컬 권한 흐름
- [x] 자동 저장, 변경 비교, 버전 스냅샷
- [ ] Supabase 실제 이메일 인증
- [ ] 서버 RLS 기반 프로젝트 접근 제한
- [ ] 미리보기 전용 URL
- [ ] 업종별 공개 URL 생성
- [ ] 이전 버전 원클릭 롤백
- [ ] 공개 후 캐시 무효화
...
- [x] React와 Vite 기반 반응형 랜딩페이지
- [x] 02B Editorial 디자인 토큰
- [x] 중앙 정렬 무한 루프 캐러셀
- [x] 캐러셀 최소 3장 데이터 검증
- [x] 스크롤 연동 텍스트·이미지 모션
- [x] 데스크톱·태블릿·모바일 iframe 미리보기
- [x] Zod 사이트 데이터 검증
- [x] LocalStorage 초안·공개본·최대 20개 버전
- [x] IndexedDB 이미지 업로드와 미디어 선택
- [x] JSON 가져오기·내보내기
- [x] 로컬 로그인과 세 역할 권한 UX
- [x] Supabase 테이블·RLS 초안
- [x] R2 저장 규칙과 환경변수 경계 문서

### 완료 조건

CMS V2 MVP는 다음 조건을 모두 만족해야 완료로 판단합니다.

1. 새 프로젝트에서 네 업종 중 하나를 선택할 수 있어야 합니다.
2. 업종 선택 후 필요한 메뉴와 기본 콘텐츠가 자동 생성되어야 합니다.
3. 회사·레스토랑·웨딩 템플릿이 각각 데스크톱 1440px, 태블릿 768px, 모바일 390px에서 정상 작동해야 합니다.
4. 한 프로젝트의 콘텐츠와 미디어가 다른 프로젝트에서 보이지 않아야 합니다.
5. 기존 첼로 프로젝트가 데이터 손실 없이 `lesson` 카테고리에서 열려야 합니다.
6. 소유자·편집자·검토자 권한이 서버와 화면에서 동일하게 적용되어야 합니다.
7. 공개 전 업종별 필수 필드 검증을 통과해야 합니다.
8. 공개본 생성과 이전 버전 복원이 실제 배포 URL에서 검증되어야 합니다.

### 구현 순서

1. category와 section registry
2. 기존 첼로 데이터 마이그레이션
3. 프로젝트 생성·목록·전환
4. registry 기반 공통 편집기
5. 회사 템플릿
6. 레스토랑 템플릿
7. 웨딩 템플릿
8. Supabase·R2 연결
...
- `main`: 현재 첼로 랜딩 CMS의 검증된 기준 스냅샷
- `CMSV2`: 다중 업종 CMS 설계와 구현 브랜치
- CMS V2 체크박스는 구현과 필요한 검증을 모두 통과한 경우에만 `[x]`로 변경합니다.
...
| 기준              | 커밋      | 내용                                        |
| ----------------- | --------- | ------------------------------------------- |
| `main` 현재 기준  | `1da678c` | 첼로 CMS 기준선과 GitHub 브랜치 운영 현황판 |
...
| 단계                            | 상태       | 현재 결과                                                 | 완료 기준                                  |
| ------------------------------- | ---------- | --------------------------------------------------------- | ------------------------------------------ |
| 1. 기존 레슨 CMS 기반           | ✅ 완료    | 권한, 로컬 저장, 미디어, 반응형 미리보기 구현             | 빌드와 브라우저 회귀 검증                  |
| 2. 다중 카테고리 기반           | 🟡 진행 중 | category·section registry와 레슨 데이터 마이그레이션 구현 | 프로젝트 생성·전환과 registry 편집기       |
| 3. 회사·레스토랑·웨딩 템플릿    | ⚪ 대기    | 업종별 필수·선택 섹션 정의                                | 세 템플릿 반응형 QA                        |
| 4. 서버 저장·R2                 | ⚪ 대기    | Supabase 스키마, RLS 정책, 환경변수 계약 작성             | 실제 인증·DB·R2 통합 테스트                |
| 5. 공개 배포                    | ⚪ 대기    | 로컬 공개본과 버전 스냅샷만 제공                          | 업종별 URL·미리보기·롤백 동작              |
| 6. 업종별 고급 기능과 상담 관리 | ⚪ 대기    | 상담 폼 UI와 외부 링크 자리만 제공                        | 업종별 문의·예약·RSVP 저장과 개인정보 관리 |
...
CMS V2의 다중 카테고리 기반을 설계한 상태입니다.

- 업종 모델: `lesson`, `company`, `restaurant`, `wedding`
- 확장 방식: category registry와 section registry
- 기존 프로젝트 보호: `ongyeol-cello`를 `lesson` 카테고리로 마이그레이션
- 구현 완료: 프로젝트 모델, registry, 기존 레슨 데이터 자동 마이그레이션
- 다음 구현 대상: 새 프로젝트 생성·목록·전환
- 서버 연결 기반은 `supabase/schema.sql`, `docs/CLOUD_MVP.md`, `.env.example`에 보존
...
1. 프로젝트 모델에 `category`와 `templateId` 추가
2. category registry와 section registry 구현
3. 기존 첼로 데이터를 `lesson` 구조로 마이그레이션
4. 새 프로젝트 생성·목록·전환 구현
5. registry 기반 공통 편집기 구현
...
| CMS V2 registry·migration | ✅ 통과   | `npm run test:cms-v2`                 |
