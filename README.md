...
> 마지막 업데이트: 2026-08-03
>
> 현재 단계: **Local CMS 안정화 · 레스토랑/프랜차이즈 템플릿 분리 · 자체 Fastify/PostgreSQL 백엔드 React 연결 준비**
>
...
| 경로 | 책임과 협업 시 주의점 |
| --- | --- |
| `src/App.jsx` | 학원·레스토랑·프랜차이즈를 `category/template/data shape`로 안전하게 판별하고, 불일치 데이터는 오류 화면으로 차단합니다. |
| `src/RestaurantTemplate.jsx` | 레스토랑과 프랜차이즈의 Hero·메뉴/운영 기반·지점·갤러리·문의 공통 렌더러이며 프랜차이즈 운영 이미지는 클릭형 핫스팟으로 표시합니다. |
| `src/data/hotspot.js` | PC·태블릿·모바일별 핫스팟 좌표·설명창 폭·방향 기본값과 5~95% 안전 범위를 관리합니다. |
| `src/data/template-registry.js` | 직접 URL과 CMS 미리보기가 공유하는 템플릿 ID·별칭·카테고리 해석 기준입니다. |
| `src/data/site-schema.js` | 학원·레스토랑·프랜차이즈 판별 스키마와 갤러리·미디어·초점 검증 계약입니다. |
| `src/data/restaurant-site.js` | `restaurant-editorial-01`의 메뉴·예약·공간 갤러리 기본 콘텐츠입니다. |
| `src/data/franchise-site.js` | 무인 카페·소형 리테일에도 적용 가능한 프랜차이즈 기본 콘텐츠와 지점 데이터입니다. |
...
- [x] 학원 02B와 레스토랑을 유지하면서 `franchise-brand-01`을 독립 카테고리와 스키마로 구현
- [x] 레스토랑 Hero 이미지/MP4 계약과 갤러리 최소 3장 검증
- [x] `restaurant` 주소 별칭·실제 템플릿 ID·카테고리 주소를 공통 레지스트리에서 해석하고 잘못된 주소 안내
- [x] 레스토랑을 풀블리드 Hero·번호형 메뉴·중앙 무한 순환 갤러리·예약 흐름의 `NIGHT TABLE` 에디토리얼 디자인으로 재구성
- [x] 프랜차이즈 고정 헤더·가맹 경쟁력·절차·운영 기반·복수 지점·매장 사진·모바일 문의 CTA 구현
- [x] CMS 지점명·상태·주소·운영시간·사진·초점 편집과 1~30개 지점 추가/삭제 구현
- [x] 프랜차이즈 핵심 섹션을 `100svh` 최소 높이와 `vh` 기반 여백·이미지 크기로 통일
- [x] 프랜차이즈에 첫 진입·스크롤 등장·직접 조작 3단계 모션과 모션 감소 대응 구현
- [x] 초광폭·저높이 화면에서 Hero 제목 크기와 고정 헤더 가시성을 보정
- [x] 프랜차이즈를 보틀 그린·아이보리·46:54 분할 Hero와 절제된 산세리프 중심의 Quiet Editorial 콘셉트로 재구성
- [x] 경쟁력을 `브랜드 선언 + 세로형 3단계 가치 목록`으로 재구성하고, 5단계 현재 섹션 헤더·전체 화면 모바일 메뉴·인터랙티브 구간 CTA 중복 방지와 고대비 폴백으로 시안 마감
- [x] 헤더·Hero·운영 선택·갤러리·문의·모바일 CTA의 기본/hover/focus 글자·배경 대비와 화살표 피드백을 단일 상호작용 규칙으로 통일
- [ ] 프랜차이즈 운영 기반을 `대표 이미지 1장 + 클릭형 설명 포인트`로 전환하고 CMS에 PC·태블릿·모바일별 드래그 좌표·설명창 폭·방향 편집 구현 — 코드·단위 검증 완료, Chrome 실화면 검증 대기
...
- [ ] 레스토랑·프랜차이즈 템플릿을 서버 프로젝트 데이터와 StorageProvider에 연결
...
- 템플릿과 CMS는 Local 데이터까지 연결됐지만, 실제 사용자 프로젝트·PostgreSQL·객체 스토리지와는 아직 연결되지 않았습니다.
...
| 검증 | 결과 | 의미 |
| --- | --- | --- |
| `npm test` | ✅ 76/76 통과 | 학원·레스토랑·프랜차이즈 스키마, 반응형 핫스팟, 레지스트리, 렌더러와 자체 API/Auth/DB 경계 회귀 |
| `npm run build` | ✅ 통과 | 현재 작업 브랜치 React/Vite 프로덕션 빌드 |
| 레스토랑 반응형·상호작용 | ✅ Chromium 실화면 검증 | 1440×960·768×1024·390×844에서 가로 넘침 0, 콘솔 오류 0건, 모바일 메뉴 열기·닫기와 중앙 순환 갤러리 `01/03 → 02/03` 확인 |
| 프랜차이즈 기본 반응형·모션 | ✅ 이전 Chrome 실화면 검증 | 1440×900·768×1024·390×844에서 헤더·경쟁력 패널·문의 CTA 대비와 가로 넘침 0을 확인한 기준선입니다. 기존 1862×558 저높이 보정 유지. Safari·Firefox 실기기 검증은 별도 필요 |
| 프랜차이즈 운영 이미지 핫스팟 | 🟡 코드·단위 검증 | 대표 이미지 1장, 설명 포인트 4개, 한 번에 하나만 여는 팝업, 키보드 접근, 화면별 5~95% 좌표·180~360px 설명창 계약과 CMS 드래그 편집 구현. Chrome이 닫혀 있어 1440·768·390 실화면 및 실제 드래그 검증 대기 |
| 학원/레스토랑/프랜차이즈 CMS 전환 | 🟡 코드·단위 검증 | 공통 미리보기 경로와 카테고리별 탭 구현. 실제 브라우저 CMS 전환 확인 필요 |
...
| 레스토랑 템플릿 | `http://127.0.0.1:4318/?template=restaurant` | `restaurant-editorial-01` 직접 확인 |
| 프랜차이즈 템플릿 | `http://127.0.0.1:4318/?template=franchise` | `franchise-brand-01` 직접 확인 |
...
`127.0.0.1`은 `npm run dev`가 실행 중인 **같은 컴퓨터에서만** 열리는 개발 주소입니다. 주소를 복사하는 것만으로 서버가 시작되지는 않으며, 다른 PC·휴대폰 공유에는 LAN 바인딩 또는 실제 배포 URL이 필요합니다.
